...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # initially the decoder input is simply sos
    ...
    # build mask for the target
        ...
        # calculate output from the decoder
...
def get_all_sentences(ds, lang):
    for item in ds: # every item is a pair of sentences (one english, one italian)
        yield item['translation'][lang]

def get_or_build_tokenizer(config, ds, lang): # configuration of the model, dataset, language
    tokenizer_path = Path(config['tokenizer_file'].format(lang))
    if not Path.exists(tokenizer_path):
        tokenizer = Tokenizer(WordLevel(unk_token="[UNK]")) # tokens not in the vocabulary
        tokenizer.pre_tokenizer = Whitespace()
        trainer = WordLevelTrainer(special_tokens=["[UNK]", "[PAD]", "[SOS]", "[EOS]"], min_frequency=2) # min frequency to add a word in the vocabulary
        tokenizer.train_from_iterator(get_all_sentences(ds, lang), trainer=trainer) # retrieves sentences from dataset
...
def train_model(config):
    ...
    # Tensorboard (visualize the loss)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        batch_ierator = tqdm(train_dataloader, desc = f"Processing Epoch {epoch:02d}")
        for batch in batch_ierator:
            model.train()
...
if __name__ == '__main__':
    ...
    config = get_config(preload="28") # set to None to start from scratch, or to a string like '02' to load the model from epoch 2
    train_model(config)
